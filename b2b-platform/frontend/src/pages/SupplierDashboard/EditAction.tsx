...
const EditAction: React.FC = () => {
    ...
    const loadAction = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          starts_at: formatDateForInput(actionData.starts_at),
          ends_at: formatDateForInput(actionData.ends_at),
          is_active: actionData.is_active,
        });
      } catch (error: any) {
        console.error('Ошибка загрузки акции:', error);
        toast.error('Ошибка при загрузке акции');
        navigate('/dashboard/actions');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiService.put(`/ads/actions/${id}/`, submitData);
        ...
        <div className="flex items-center mb-6">
          ...
          <div>
            ...
            <p className="text-dark-300 text-lg mt-2">
              Обновите информацию о вашем специальном предложении
            </p>
          </div>
        </div>
